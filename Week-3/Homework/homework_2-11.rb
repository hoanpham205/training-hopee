...
staffs = [{ id: 1, positions: %w[tonggiamdoc truongphong] },
          { id: 2, positions: %w[giamdoc nhanviensale truongphong] }]

def sort_staffs(staffs)
  positions = { 'tonggiamdoc' => 0, 'giamdoc' => 1, 'truongphong' => 2, 'nhanvienvp' => 3, 'nhanviensale' => 4 }
  staffs.sort_by do |staff|
    p priority = staff[:positions].min_by { |pos| positions[pos] }
    p staff[:positions].sort_by! { |position| positions[position] }
    p [positions[priority], staff[:id]]
  end
end
